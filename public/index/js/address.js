...
addressBox += '<form id="address"><div class="form-section form-name">';
...
!function($){
    ...
    $('#J_modalEditAddress').on('click','#J_cancel,.close',function(){
    ...
    $('#J_modalEditAddress').on('change','#J_county',function(){
        var index = $(this).find('option:selected').index()-1;
        if(index>-1){
            inpData.county = $(this).find('option:selected').text();
        }else{
            inpData.county = '';
        }
        // console.log(inpData);
    ...
    $('#J_modalEditAddress').on('click','#J_save',function(){
        ...
        if(e&&inpData.province&&inpData.city&&inpData.county){
            $.ajax({
                url:'/user/address',
                data:inpData,
                dataType:'json',
                success:function(data){
                    if(data.status == 200){
                        createAddress(inpData,data.ress);
        ...
        }else if(!(inpData.province&&inpData.city&&inpData.county)){
    ...
    function createAddress(data,aid){
        var html = '';
        html += '<div class="address-item J_addressItem selected" data-address_id="' + aid + '">';
        html += '<dl><dt><em class="username">这些擦</em></dt>';
        html += '<dd class="utel">12312312312</dd>';
        html += '<dd class="uaddress"></dd></dl>';
        html += '<div class="actions">';
        html += '<a href="javascript:void(0);" class="modify J_addressDel">删除</a>';
        html += '</div></div>';
        var $newAddress = $(html).insertBefore($('#J_addressList'));
        $newAddress.data('address_id',aid);
        $newAddress.data('consignee',data.user_name);
        ...
        $newAddress.find('.username').html(data.user_name);
        $newAddress.find('.utel').html(data.user_phone);
        $newAddress.find('.uaddress').html(data.province+' '+data.city+' '+data.county+' <br>'+data.user_adress);
        $('#J_confirmAddress').html(data.user_name+' '+data.user_phone+'<br>'+data.province+' '+data.city+' '+data.country+' <br>'+data.address);
        $('.J_addressItem.selected').eq(1).removeClass('selected');
    }
